...
data3 <- data2 %>%
  select(HHLFD, INTNR, east,GESCHLECHT, XALTER, h11008, h11008_mu, h11008_va, h11008_omamu, h11008_opamu, h11008_omava, h11008_opava,
...
columns_to_convert <- c("Alter_der_Person_in_Jahren_Mutter_", "Alter_der_Person_in_Jahren_Vater_", "vereinbarte_Arbeitszeit_Mutter_", "vereinbarte_Arbeitszeit_Vater_", "Trennungsjahr_der_Eltern_Mutter_")
...
data <- data %>% 
  mutate(across(where(is.numeric), ~ ifelse(is.infinite(.), median(., na.rm = TRUE), .)))
# colnames(data)
# alias_vars <- c("BLand_","Aktivitätsstatus_Mutter_")
# data <- data %>% select(-all_of(alias_vars))
...
data1 <- data %>%
  filter(Alter_der_Person_in_Jahren_ > 6)
...
trainIndex <- createDataPartition(data2$dep_child, p = .8,
                                  list = FALSE,
                                  times = 1)
